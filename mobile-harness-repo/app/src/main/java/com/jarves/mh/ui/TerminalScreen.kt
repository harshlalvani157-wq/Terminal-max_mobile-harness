                    ...
                    SelectionContainer {
                        Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                            ...
                            lines.forEach { item ->
                                Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
                                    ...
                                    if (item.output.isNotEmpty()) {
                                        val defaultColor = if (item.exitCode != 0) MaterialTheme.colorScheme.error else outputTextColor
                                        val formatted = remember(item.output, defaultColor) {
                                            parseAnsiTerminalOutput(item.output, defaultColor)
                                        }
                                        Text(
                                            text = formatted,
                                            fontFamily = FontFamily.Monospace,
                            ...
                            if (liveOutput.isNotBlank()) {
                                val liveFormatted = remember(liveOutput, outputTextColor) {
                                    parseAnsiTerminalOutput(liveOutput, outputTextColor)
                                }
                                Text(
                                    text = liveFormatted,
                                    fontFamily = FontFamily.Monospace,
...
internal fun parseAnsiTerminalOutput(raw: String, defaultColor: Color): AnnotatedString {
    ...
    for (match in matches) {
        ...
        if (start > currentIndex) {
            val textSegment = raw.substring(currentIndex, start)
                .replace(Regex("""\u001B\[[0-9;?]*[A-Za-z]"""), "")
            if (textSegment.isNotEmpty()) {
                val spanStyle = SpanStyle(
                    color = if (isDim) currentColor.copy(alpha = 0.65f) else currentColor,
        ...
        if (codeString.isEmpty() || codeString == "0") {
            ...
            while (i < codes.size) {
                when (val code = codes[i]) {
                    ...
                    in 30..37 -> currentColor = ansi16Color(code - 30, isBright = false)
                    in 90..97 -> currentColor = ansi16Color(code - 90, isBright = true)
                    38 -> {
                        if (i + 2 < codes.size && codes[i + 1] == 5) {
                            currentColor = ansi256Color(codes[i + 2])
                            i += 2
    ...
    if (currentIndex < raw.length) {
        val remaining = raw.substring(currentIndex)
            .replace(Regex("""\u001B\[[0-9;?]*[A-Za-z]"""), "")
...
private fun ansi16Color(index: Int, isBright: Boolean): Color {
    return when (index) {
        0 -> if (isBright) Color(0xFF767676) else Color(0xFF2C2C2C)
        1 -> if (isBright) Color(0xFFFF6B68) else Color(0xFFE04441)
        2 -> if (isBright) Color(0xFF5AF78E) else Color(0xFF27AE60)
        3 -> if (isBright) Color(0xFFF3F99D) else Color(0xFFE5C07B)
        4 -> if (isBright) Color(0xFF57C7FF) else Color(0xFF4A90E2)
        5 -> if (isBright) Color(0xFFFF79C6) else Color(0xFF9B59B6)
        6 -> if (isBright) Color(0xFF9AEDFE) else Color(0xFF1ABC9C)
        7 -> if (isBright) Color(0xFFF1F1F0) else Color(0xFFDCDFE4)
...
private fun ansi256Color(index: Int): Color {
    return when {
        index in 0..7 -> ansi16Color(index, isBright = false)
        index in 8..15 -> ansi16Color(index - 8, isBright = true)
        index in 16..231 -> {
            val n = index - 16
            val r = (n / 36) * 51
            val g = ((n % 36) / 6) * 51
